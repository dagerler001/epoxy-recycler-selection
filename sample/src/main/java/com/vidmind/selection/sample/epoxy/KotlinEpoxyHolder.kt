...
abstract class KotlinEpoxyHolder : EpoxyHolder() {
    ...
    protected fun <V : View> bind(parentViewId: Int, childViewId: Int): ReadOnlyProperty<KotlinEpoxyHolder, V> =
        Lazy { holder: KotlinEpoxyHolder, prop ->
            val parentView = (holder.view.findViewById(parentViewId) as View?)
            parentView?.findViewById(childViewId) as V?
                ?: throw IllegalStateException("View ID $childViewId for '${prop.name}' not found.")
        }
